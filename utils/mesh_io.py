...
def load_mesh(surf_mesh,correct_offset=False):
    ...
    if isinstance(surf_mesh, str):
        if (surf_mesh.endswith('orig') or surf_mesh.endswith('pial') or
            ...
            coords, faces = nb.load(surf_mesh).getArraysFromIntent(nb.nifti1.intent_codes['NIFTI_INTENT_POINTSET'])[0].data, \
                            nb.load(surf_mesh).getArraysFromIntent(nb.nifti1.intent_codes['NIFTI_INTENT_TRIANGLE'])[0].data
