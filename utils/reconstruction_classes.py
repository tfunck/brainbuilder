...
class SlabReconstructionData():
    def __init__(self, brain, hemi, slabs, ligands, depths,  volume_dir, surface_dir, resolution):
        self.brain=brain
        self.hemi=hemi
        ...
        self.slabs = np.sort( np.array(slabs).astype(int) )
    ...
    def create_values_raw_dict(self, tissue_type=''):
        ...
        for ligand in self.ligands : 
            ...
            for depth in self.depths :
                file_dict[ligand][depth]=f'{self.volume_dir}/{self.brain}_{self.hemi}_{ligand}_{self.resolution}mm{tissue_type}_l{self.n_depths}_{depth}_raw.csv'
        return file_dict

    def create_values_interp_dict(self, tissue_type=''):
        file_dict={}
        for ligand in self.ligands : 
            file_dict[ligand]=f'{self.volume_dir}/{self.brain}_{self.hemi}_{ligand}_{self.resolution}mm{tissue_type}_l{self.n_depths}.h5'
        return file_dict
    ...
    def gen_surface_filename(self, slab, depth):
        return f'{self.surface_dir}/slab-{slab}_surf_{self.resolution}mm_{depth}_rsl.npz'

    def gen_volume_filename(self,slab,ligand,tissue_type=''):    
        return f'{self.volume_dir}thickened{tissue_type}_{int(slab)}_{ligand}_{self.resolution}_l{self.n_depths}.nii.gz'
    ...
    def create_volume_dict(self, tissue_type=''):
        ...
        for ligand in self.ligands :
            ...
            for slab in self.slabs:
                files[ligand][slab] = self.gen_volume_filename(slab, ligand, tissue_type=tissue_type)
    ...
    def create_spheres_dict(self):
        ...
        for slab in self.slabs:
            files[slab]={}
            for depth in self.depths:
                files[slab][depth] = self.gen_sphere_filename( depth)
        return files
    ...
    def create_surface_dict(self):
        ...
        for slab in self.slabs:
            files[slab]={}
            for depth in self.depths:
                files[slab][depth] = self.gen_surface_filename(slab, depth)
        return files
    ...
    def set_mid_surfaces(self):
        ...
        for slab in self.slabs :
            mid_surfaces_dict[slab] = self.surfaces[slab][self.middle_depth]

        return mid_surfaces_dict 

    def get_surfaces_across_slabs(self, depth):
        return [ self.surfaces[s][depth] for s in self.slabs  ]
