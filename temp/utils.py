...
def safe_ants_image_read(fn, tol=0.001, clobber=False):
    img = nib.load(fn)  
    ndim=img.ndim
    origin = list(img.affine[ [0,1,2],[3,3,3] ])[0:ndim]
    spacing = list( img.affine[ [0,1,2],[0,1,2] ])[0:ndim]
...
def points2tfm(points_fn, affine_fn, ndim=3, transform_type="Affine", invert=False, reference='', clobber=False):
    ...
    if not os.path.exists(affine_fn) or clobber :
        ...
        if reference != '' :
            com0 = np.array(center_of_mass(moving_points) )
            com1 = np.array(ants.get_center_of_mass( safe_ants_image_read(reference) ) )
            delta=com1-com0
            #fixed_points += delta 
            moving_points += delta
            print(center_of_mass(moving_points), com1)

        print('\tCalculate affine matrix from points :', transform_type)

        landmark_tfm = ants.fit_transform_to_paired_points( moving_points, fixed_points, transform_type=transform_type)
        #print('fixed points')
        #print(fixed_points)

        tfm_points =[ ants.apply_ants_transform(landmark_tfm, fixed_points[i]) for i in range(fixed_points.shape[0])]
        tfm_points = np.array(tfm_points)
        ...
        #print('moving points')
        #print(moving_points)
        loss = np.sum(np.sqrt(np.sum( np.power( (tfm_points - moving_points), 2), axis=1)))
        print('loss =',loss)
        print(landmark_tfm.fixed_parameters)
