...
for mr in MR1 ; do
	for h in R ; do 
  		for slab in slab_1 slab_2 ; do
			python3 detectLines.py  --epochs 5 --step 0.2 --train-source "test/" --train-output model_results/
			
			#Step 1 : Apply line removal
			python3 detectLines.py --step 0.2 --train-source "test/" --train-output model_results/  --raw-source raw/${h}_$slab --raw-output ${mr}/${h}_${slab}/lines_removed 
