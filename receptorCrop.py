...
def crop_source_files(source_files, output_dir, downsample_step=0.5, clobber=False, manual_check=False, manual_only=False) :
    qc_status=0
    for f in source_files :
        ...
        if (not os.path.exists(fout) or not os.path.exists(qc_fn)) or clobber :
            ...
            if not manual_only :
                bounding_box_dwn, img_smoothed,  im_thr, cc = cropp_img(img_dwn)
                #Crop image
