...
if __name__ == '__main__':
    ...
    parser.add_argument('--autoradiograph-fn','-a', dest='auto_df_fn', default='section_numbers/autoradiograph_info.csv', type=str,  help='Slabs to reconstruct. Default = reconstruct all slabs.')
    ...
    autoradiograph_info_fn='autoradiograph_info_volume_order.csv'

    #Process the base autoradiograph csv
    if not os.path.exists(autoradiograph_info_fn) : 
        calculate_section_order(autoradiograph_info_fn, crop_dir,in_df_fn=args.auto_df_fn)
    ...
    ### Step 0 :
    crop(src_dir,crop_dir, df, 'autoradiograph_info_downsample.csv')
    autoradiograph_info_fn='autoradiograph_info_downsample.csv'
    ...
    for brain in brain_list :
        for hemi in hemi_list : 
            for slab_i, slab in enumerate(slab_list) : 
                ...
                if not os.path.exists( init_align_fn) or True  : 
            ...
            interp_dir=f'{out_dir}/6_surf_interp/'
            ...
            surface_interpolation(nl_tfm_list,  nl_2d_list, interp_dir, brain, hemi, resolution, autoradiograph_info_fn, n_depths=1)
