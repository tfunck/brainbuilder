...
def radon_tfm(ii, forward=True):
    theta = np.linspace(0., 180., max(ii.shape)*2, endpoint=False)
    if forward :
        out = radon(ii, theta=theta, circle=True)
    else :
        out = iradon(ii, theta=theta, circle=True)
    '''
    plt.subplot(1,2,1)
    plt.imshow(ii)
    plt.subplot(1,2,2)
...
def main(source_dir, output_dir, step, epochs, clobber) :
    ...
    if not os.path.exists(source_dir+os.sep+'train.npy') or not os.path.exists(source_dir+os.sep+'labels.npy') or clobber  :
        ...
        np.save(source_dir+os.sep+'train', images)
        np.save(source_dir+os.sep+'labels', masks)
    else : 
