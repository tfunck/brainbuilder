...
class Simulation():
    ...
    def __init__(self, 
        ...
        self.vtx_values_corrected_filename = f'{out_dir}/surface_values_corrected_{epoch}'
        ...
        self.labels = self.create_batches(chunk_reduction=chunk_reduction, clobber=clobber)

        # generate random surface values
        self.vtx_values, self.slope = self.generate_random_surface_values(clobber=clobber)
        ...
        self.vtx_values_batch = self.add_batch_effects(use_batch_effects=True, clobber=clobber)
    ...
    def add_noise(self, noise_ratio, use_noise=True, clobber=False):
        ...
        if use_noise:
            if not os.path.exists(self.vtx_values_noise_filename+'.npz') or clobber :
                ...
                visualization(self.surf_coords_filename, noisy_orig, self.noise_qc_filename) 
            else :
                img=np.load(self.vtx_values_noise_filename+'.npz')
                nsr=img['nsr']
        else :
            shutil.copy(self.vtx_values_orig_filename+'.npz', self.vtx_values_noise_filename+'.npz')
            nsr = 0
    ...
    def apply_correction(self, params, unique_paired_values):
        ...
        vtx_values_corrected[idx] = vtx_values_corrected[idx] - np.mean(vtx_values_corrected[idx]) + approx_mean

        visualization(self.surf_coords_filename, vtx_values_corrected, self.corrected_qc_filename)
    ...
    def create_batches(self, border_offset=5, chunk_reduction=0.1, clobber=False):
        ...
        if not os.path.exists(self.label_values_filename+'.npz') or clobber :
            ...
            for i, (ylimc, ylimr) in enumerate(zip(percentiles[:-1:], percentiles[1::])):
                ...
                assert np.sum(idx) > 0 , 'Error: no vertices between rostral and caudal limits'
    ...
    def add_batch_effects(self, use_batch_effects=True, clobber=False):
        ...
        if not os.path.exists(self.vtx_values_batch_filename+'.npz') or clobber :
            if use_batch_effects : 
                ...
                scale_factor = np.ones(self.n_slabs)# np.random.normal(0.5,1.5,self.n_slabs)
                ...
                for i, slab in enumerate(np.unique(self.labels)[1:]) :
                    ...
                    vtx_values_batch[ idx ] = offset_factor[i] + scale_factor[i] * self.vtx_values[idx]

                np.savez(self.vtx_values_batch_filename, data=vtx_values_batch)
                visualization(self.surf_coords_filename, vtx_values_batch, self.batch_qc_filename) 
            
            else :
                shutil.copy(self.vtx_values_orig_filename+'.npz', self.vtx_values_batch_filename+'.npz')
...
def plot_results(df, dist_df):
    ...
    plt.figure(figsize=(10,10))
    ax = sns.scatterplot(x='Distance (mm)', y='Sagittal Gradient Size', hue='r2', data=df, palette='nipy_spectral_r')
    out_png = f'./batch_correction/dist_gradient.png'
    ax.figure.savefig(out_png)
    ax.figure.clf()
    ...
    out_png = f'./batch_correction/r2_hist.png'
    plt.savefig(out_png)
    plt.clf()
...
def calculate_average_distances(paired_values_files, surface_filename ):
    ...
    results = Parallel(n_jobs=8)(delayed(calc_distance)(f, coords) for f in paired_values_files)    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--clobber', '-c', dest='clobber',default=False,action='store_true', help='Overwrite existing results')
    args = parser.parse_args()


    n_epochs=1000
    
    existing_results = glob.glob(f'./batch_correction/*/results_*.csv')
    ...
    surf_coords_fn=f'data/mr1/MR1_gray_surface_R_81920.surf.gii'
    ...
    Parallel(n_jobs=n_jobs)(delayed(Simulation)(
        epoch, 
        sphere_coords_fn, 
        surf_coords_fn,  
    ...
    results_files = glob.glob(f'./batch_correction/*/results_*.csv')
    paired_values_files = glob.glob(f'./batch_correction/*/paired_values.csv')

    dist_df_fn = f'./batch_correction/dist_df.csv'
