...
def save_gii(coords, triangles, reference_fn, out_fn):
    ...
    ar_triangle = nb.gifti.gifti.GiftiDataArray(
...
def save_mesh_data(fname, surf_data):
    if isinstance(fname, str) and isinstance(surf_data, np.ndarray):
        if (
            fname.endswith("curv")
            or fname.endswith("thickness")
            or fname.endswith("sulc")
        ):
            ...
            if "data" in surf_dict.keys():
                write_vtk(
                    fname, surf_dict["coords"], surf_dict["faces"], surf_dict["data"]
                )
            else:
            ...
            print(
                "please write lovely write mgh command, or retry saving as .curv file"
            )
    else:
...
def read_vtk(file):
    ...
    try:
        ...
        raise ValueError(
            "This vtk file appears to be binary coded currently only ASCII coded vtk files can be read"
        )
    vtk_df = vtk_df.dropna()
    # extract number of vertices and faces
    number_vertices = int(
        vtk_df[vtk_df[0].str.contains("POINTS")][0].iloc[0].split()[1]
    )
        ...
        for row in range(1, (int(number_vertices / 3) + 1)):
            for col in [0, 3, 6]:
                vertex_array = np.append(
                    vertex_array,
                    np.array(vertex_df.iloc[row : (row + 1), col : (col + 3)]),
                    axis=0,
                )
...
def read_ply(file):
    ...
    try:
        ...
        raise ValueError(
            "This ply file appears to be binary coded currently only ASCII coded ply files can be read"
        )
    ply_df = ply_df.dropna()
    # extract number of vertices and faces, and row that marks the end of header
    number_vertices = int(
        ply_df[ply_df[0].str.contains("element vertex")][0].iloc[0].split()[2]
    )
    number_faces = int(
        ply_df[ply_df[0].str.contains("element face")][0].iloc[0].split()[2]
    )
    end_header = ply_df[ply_df[0].str.contains("end_header")].index.tolist()[0]
    # read vertex coordinates into dict
    vertex_df = pd.read_csv(
        file,
        skiprows=range(end_header + 1),
        nrows=number_vertices,
        sep="\s*",
        header=None,
        engine="python",
    ...
    if isinstance(fname, str) and isinstance(surf_dict, dict):
        if (
            ...
            nb.freesurfer.io.write_geometry(
                fname, surf_dict["coords"], surf_dict["faces"]
            )
