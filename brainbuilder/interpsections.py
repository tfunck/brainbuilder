...
from brainbuilder.interp.surfinterp import generate_surface_profiles, create_final_reconstructed_volume, write_raw_profiles_to_volume
...
def surface_pipeline(
    ...
    if not os.path.exists(reconstructed_cortex_fn) or \
            (use_surface_smoothing and not os.path.exists(smoothed_reconstructed_cortex_fn)) or\
            clobber :
        ...
        if batch_correction_resolution > 0 : 
            sect_info, batch_surf_raw_values_dict, final_profiles_fn = get_profiles_with_batch_correction(
                chunk_info,
                sect_info,
                chunk_info_thickened_csv,
                sub,
        ...
        if use_surface_smoothing :
            ...
            smoothed_final_profiles_fn = smooth_surface_profiles(final_profiles_fn, surf_depth_mni_dict, sigma, clobber=clobber)
...
def interpolate_missing_sections(
    ...
    for (acquisition, chunk), chunk_info_row in sect_info.groupby(["acquisition", "chunk"]):
